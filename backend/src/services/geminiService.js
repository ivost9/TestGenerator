...
// ❌ МАХАМЕ "const ai = new GoogleGenAI(...)" от тук!
  ...
  try {
    ...
    const responseSchema = {
      ...
      properties: {
        isValidTopic: {
          type: Type.BOOLEAN,
          description: "Върни true, ако темата е реална...",
        },
    ...
    const prompt = `
      ...
      Състави официален училищен тест по:
      - Предмет: ${subject}
      - Клас: ${grade} клас
      - Тема: ${topic}
      - Трудност: ${difficulty}
    ...
    console.log(`🤖 Извикване на Gemini за тема: "${topic}"...`);

    // Викаме модела през локално създадения "ai" клиент
    const response = await ai.models.generateContent({
      model: "gemini-1.5-flash", // Оставаме на сигурния 1.5-flash за момента
      contents: prompt,
      config: {
        responseMimeType: "application/json",
        responseSchema: responseSchema,
    ...
    if (!result.isValidTopic) {
      throw new Error(result.validationMessage || `Темата не съответства.`);
    }
